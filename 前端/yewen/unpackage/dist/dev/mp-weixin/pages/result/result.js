  ...
  onLoad(options) {
    if (options.data) {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/result/result.vue:229", "解析数据失败", e);
      }
    }
    if (options.imageUrl) {
  ...
  methods: {
    ...
    initSpeech() {
      if (window.speechSynthesis) {
        ...
        common_vendor.index.__f__("log", "at pages/result/result.vue:261", "当前浏览器不支持语音合成");
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    B: common_vendor.o((...args) => $options.openAssistant && $options.openAssistant(...args), "7f"),
    C: $data.showToast
  }, $data.showToast ? {
    D: common_vendor.t($data.toastText)
  } : {});
}
