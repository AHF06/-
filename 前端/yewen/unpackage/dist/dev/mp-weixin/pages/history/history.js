  ...
  computed: {
    ...
    filteredRecords() {
      ...
      if (this.searchKeyword) {
        records = records.filter((r) => r.diseaseName.includes(this.searchKeyword) || r.cropName.includes(this.searchKeyword));
      }
...
防治建议：${item.solution || "建议咨询植保专家"}`,
    ...
    generateReport() {
      ...
      this.monthlyTrend = Array.from(monthMap.entries()).sort((a, b) => a[0].localeCompare(b[0])).map(([name, count]) => ({ name: name.substring(5), count })).slice(-6);
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    A: common_vendor.o((...args) => $options.generateReport && $options.generateReport(...args), "71"),
    B: $data.showFilterDrawer
  }, $data.showFilterDrawer ? {
    C: common_vendor.o(($event) => $data.showFilterDrawer = false, "d4"),
    ...
    E: $data.filterCrop === "" ? 1 : "",
    F: common_vendor.o(($event) => $data.filterCrop = "", "4b"),
    ...
    I: common_vendor.o(($event) => $data.filterSeverity = "", "84"),
    J: $data.startDate,
    K: common_vendor.o(($event) => $data.startDate = $event.detail.value, "0b"),
    L: $data.endDate,
    M: common_vendor.o(($event) => $data.endDate = $event.detail.value, "f9"),
    N: common_vendor.o((...args) => $options.resetFilters && $options.resetFilters(...args), "b3"),
    O: common_vendor.o((...args) => $options.applyFilters && $options.applyFilters(...args), "ad"),
    P: common_vendor.o(() => {
    }, "64"),
    Q: common_vendor.o(($event) => $data.showFilterDrawer = false, "b9")
  } : {}, {
    R: $data.showDatePicker
  }, $data.showDatePicker ? {
    S: common_vendor.o(($event) => $data.showDatePicker = false, "2d"),
    T: common_vendor.o(($event) => $options.setQuickDate("week"), "87"),
    U: common_vendor.o(($event) => $options.setQuickDate("month"), "aa"),
    V: common_vendor.o(($event) => $options.setQuickDate("all"), "d1"),
    W: $data.tempStartDate,
    X: common_vendor.o(($event) => $data.tempStartDate = $event.detail.value, "ff"),
    Y: $data.tempEndDate,
    Z: common_vendor.o(($event) => $data.tempEndDate = $event.detail.value, "4c"),
    aa: common_vendor.o((...args) => $options.confirmDateFilter && $options.confirmDateFilter(...args), "a2"),
    ab: common_vendor.o(() => {
    }, "de"),
    ac: common_vendor.o(($event) => $data.showDatePicker = false, "aa")
  } : {}, {
    ad: $data.showReport
  }, $data.showReport ? {
    ae: common_vendor.o(($event) => $data.showReport = false, "21"),
    ...
    ai: common_vendor.t($data.reportData.rate),
    ...
    al: common_vendor.t($data.reportData.suggestion),
    am: common_vendor.o((...args) => $options.shareReport && $options.shareReport(...args), "ea"),
    an: common_vendor.o((...args) => $options.saveReport && $options.saveReport(...args), "bf"),
    ao: common_vendor.o(() => {
    }, "34"),
    ap: common_vendor.o(($event) => $data.showReport = false, "ad")
  } : {}, {
    aq: common_assets._imports_0,
    ar: common_vendor.o((...args) => $options.openAssistant && $options.openAssistant(...args), "1e")
  });
}
